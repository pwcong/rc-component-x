...
import defaultRaf, { cancel } from './raf';
import defaultNow from './performance-now';
import mapToZero from './map-to-zero';
import stripStyle from './strip-style';
import stepper from './stepper';
import mergeDiff from './merge-diff';
import shouldStopAnimation from './should-stop-animation';
...
function mergeAndSync(
  ...
  const newMergedPropsStyles = mergeDiff(oldMergedPropsStyles, destStyles, ((
    oldIndex,
    oldMergedPropsStyle
  ) => {
  ...
  let newCurrentStyles: any = [];
  let newCurrentVelocities: any = [];
  let newLastIdealStyles: any = [];
  let newLastIdealVelocities: any = [];
  for (let i = 0; i < newMergedPropsStyles.length; i++) {
    const newMergedPropsStyleCell = newMergedPropsStyles[i];
    let foundOldIndex: any = null;
...
export default class TransitionMotion extends React.PureComponent<
  TransitionProps,
